...
- UI: блоки Oscillators / Distortion / Filter / Envelopes, выходной `Gain` интегрирован в `Mono`; управление только крутилками и переключателями.
- UI: переключаемые языки RU/EN (без "крякозябр" и развала верстки).

## Changelog

### 1.0.0 (2026-02-28)
- Закрыт этап `M9`: проект проходит локальную сборку `Release` и авто-тест `ies_tests` (`ctest`).
- Версия проекта в CMake переведена на `1.0.0` (release freeze v1.0).
- `ROADMAP.md` приведён к финальному статусу: v1.0 закрыт, дальнейшая работа идёт по backlog / V2.
...
- M0–M9: закрыто. v1.0 сформирован и заморожен.
- Дальнейшие фичи и эксперименты ведутся в `ROADMAP_V2.md`.
